...
class DirectDownload(ImageInterface):
    ...
    def _get_credentials(self) -> tuple:
        ...
        if not arti_user or not arti_token:
            try:
                with open(os.path.expanduser(arti_token_file), 'r') as file:
                ...
                logging.error(f"Artifactory user. {arti_user}")
                logging.error(f"Artifactory token_file. {arti_token_file}")
                return '', '', ''
